...
class Network() :
            ...
            if self.brain[n_layer]["type"] == 'dense' :
                for n_neuron in range(len(Y[n_layer])) :
                    for n_synapse in range(len(Y[n_layer+1])) :
                        plt.plot([X[n_layer], X[n_layer+1]], [Y[n_layer][n_neuron], Y[n_layer+1][n_synapse]], c='black')
    ...
    def test(self,
        ...
        for output in range(outputs) :
            ...
            plt.scatter(np.linspace(1, tries, tries)+tries*output, self.output[:, output], s=5, color=(colour, 0, 0.5), label='Output n°'+str(output))
